...
| **View filters**  | React state + parser helpers                       | `App` owns `FilterState` (`fieldDetailMode` + `visibleTableIds` + `visibleGroupIds` + `showGroupBoundaries`) and resets it whenever a new schema is loaded. `defaultFilterState(schema)` chooses `full` for schemas with up to 30 tables and `table-only` above that threshold; all groups start visible. `applyFilters(schema, filterState)` removes hidden tables (per-table and group-level visibility) and any refs whose endpoints are not both visible. `isFilterActive` drives the toolbar indicator. Group filter checkboxes hide/show all member tables atomically while preserving per-table overrides. A `Show TableGroup Boundaries` toggle controls `showGroupBoundaries`.                                                                                                                              |
...
| **UI shell**      | React + Tailwind CSS                               | File upload (input fallback + File System Access API), DBML text editor panel, HUD controls outside the canvas, and the `View Filters` modal workflow. The toolbar exposes a `View Filters` button with an active-state indicator and a `Reload current file` button (enabled only when a file was loaded via the File System Access API). `ViewFiltersDialog` lets users switch between the three field-detail modes, select/unselect all tables, toggle individual table visibility, filter by TableGroup, toggle the Ungrouped set, and show/hide group boundaries — all with immediate effect on the scene.                                                                                                                |
